...
startActivity(mIntent);
	...
	public SportsModel addSportsListDetails(JSONObject obj) {
		...
		sportsModel.setStart_time(obj.optString("fixture_start"));
		sportsModel.setEnd_time(obj.optString("fixture_end"));
		sportsModel.setStudentIds(obj.optString(JTAG_STUDENT_IDS));
		sportsModel.setStatus(obj.optString("status"));
		List<String> items = Arrays.asList(obj.optString("studentIds").split("\\s*,\\s*"));
...
dialog.show();*/
		...
		dialogButtonApply.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				for (int i = 0; i < mFilterModelList.size(); i++) {
					if (mFilterModelList.get(i).getCheckedStudent()) {
						for (int j = 0; j < mSportsModelArrayListRe.size(); j++) {
							if (mSportsModelArrayListRe.get(j).getStudentIds().contains(mFilterModelList.get(i).getStudentId())) {
								if (contains(mSportsModelArrayList, mSportsModelArrayListRe.get(j).getSports_id())) {
									System.out.println("Data Exist(s) already");
							...
							}else if (mSportsModelArrayListRe.get(j).getStudentIds().equalsIgnoreCase(""))
							{
								if (contains(mSportsModelArrayList, mSportsModelArrayListRe.get(j).getSports_id())) {
					...
					{
						for (int j = 0; j < mSportsModelArrayListRe.size(); j++) {
							if (mSportsModelArrayListRe.get(j).getStudentIds().equalsIgnoreCase(""))
							{
								if (contains(mSportsModelArrayList, mSportsModelArrayListRe.get(j).getSports_id())) {
									System.out.println("Data Exist(s)");
								} else {
				...
				System.out.println("sizee::"+mSportsModelArrayList.size());
				sportsEventListAdapter.notifyDataSetChanged();
				searchEditText.setText(searchString);
				if (mSportsModelArrayList.size()>0)
				{
					mSportListView.setVisibility(View.VISIBLE);
				}
				else
				{
					mSportListView.setVisibility(View.INVISIBLE);
	...
	boolean contains(ArrayList<SportsModel> list, String name) {
		for (SportsModel item : list) {
			if (item.getSports_id().equals(name)) {
				return true;
			}
		}
		return false;
