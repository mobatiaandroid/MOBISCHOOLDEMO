...
public class PaymentDetailAdapter extends RecyclerView.Adapter<PaymentDetailAdapter.MyViewHolder> {
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View view) {
            if (view == paybuttonInstall)
            {
                ...
                if (mntripModelArrayList.get(Position).isEmiAvailable() && mInstallmentModelArrayList.get(clickedPosition).getEnable().equalsIgnoreCase("-1")) {
            ...
            else if (view == payTotalButton)
            {
                ...
                isInstallment = false;
                isFullPayment = true;
                installment_amount = mntripModelArrayList.get(Position).getRemaining_amount();
                amount = mntripModelArrayList.get(Position).getRemaining_amount();
                payment_option = "2";
                installment_id = mInstallmentModelArrayList.get(clickedPosition).getInstallment_id();
                installment = "1";
                listenerRefRead.get().onPositionClicked(getAdapterPosition(), isFullPayment, isInstallment, installment_amount, amount, payment_option, installment_id, installment, orderPaid);
    ...
    public PaymentDetailAdapter(Context mContext, ArrayList<TripListModel> mntripModelArrayList, int adapterSize, int Position, ClickListener listener) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        mInstallmentModelArrayList=mntripModelArrayList.get(Position).getInstallmentArrayList();
        int lastPositionInstallment=adapterSize-1;
        // Installment Available
        if (mntripModelArrayList.get(Position).isEmiAvailable()) {
            System.out.println("works  enter into is emi available");
            if (mInstallmentModelArrayList.size() == 1) {
                System.out.println("works  enter into is emi available and installment size is 1");
                if (mntripModelArrayList.get(Position).getPayment_status().equalsIgnoreCase("1")) {
                    System.out.println("works  enter into is emi available installment size 1 and payment done");
            ...
            {
                System.out.println("works  enter into is emi available installment size not 1 position not last position");
                ...
                holder.dateTxt.setText("(" + AppUtils.dateConversionddmmyyyy(mntripModelArrayList.get(Position).getInstallmentArrayList().get(position).getLast_payment_date()) + ")");
                ...
                {
                    System.out.println("works  enter into is emi available installment date not over");
                    if (mInstallmentModelArrayList.get(position).getPaid_status().equalsIgnoreCase("1")) {
                        System.out.println("works  enter into is emi available installment date not over payment done");
                        holder.addCal.setVisibility(View.GONE);
                        holder.paidTxt.setVisibility(View.VISIBLE);
                        holder.checked.setVisibility(View.VISIBLE);
                        holder.paybuttonInstall.setVisibility(View.GONE);
                    } else {
                        System.out.println("works  enter into is emi available installment date not over payment not done");
                ...
                if (mntripModelArrayList.get(Position).getInstallmentArrayList().get(position).getPaid_status().equalsIgnoreCase("0")) {
                    ...
                    holder.amountInstallTxt.setText(mntripModelArrayList.get(Position).getInstallmentArrayList().get(position).getPaid_amount());

                }
                if (mInstallmentModelArrayList.get(position).getEnable().equalsIgnoreCase("-1")) {
                    clickedPosition = position;
                    System.out.println("enable position" + mInstallmentModelArrayList.get(position).getEnable());
                   ...
                   /* if (clickedPosition != 0) {
                    ...
                    if (mInstallmentModelArrayList.get(position).getInst_date_status().equalsIgnoreCase("0")) {
                        ...
                        holder.paybuttonInstall.setBackground(mContext.getResources().getDrawable(R.drawable.button_blue));
                ...
                // installment is paid
                    ...
                    {
                        holder.dateTxt.setText("(" + AppUtils.dateConversionddmmyyyy(mntripModelArrayList.get(Position).getInstallmentArrayList().get(position).getPaid_date()) + ")");
                ...
                if (mntripModelArrayList.get(Position).getPayment_date_status().equalsIgnoreCase("0")) {
                    ...
                    if (mntripModelArrayList.get(Position).getPayment_status().equalsIgnoreCase("0")) {
                        ...
                        holder.payTotalButton.setBackground(mContext.getResources().getDrawable(R.drawable.button_blue));
         ...
         holder.addCal.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mntripModelArrayList.get(Position).getInstallmentArrayList().size() > 0) {
                    paymentMode = "1";
                    title = mntripModelArrayList.get(Position).getTitle() + "Installment " + (position + 1);
                    last_payment_date = mInstallmentModelArrayList.get(position).getLast_payment_date();


                } else {
                    paymentMode = "2";
                    title = mntripModelArrayList.get(Position).getTitle();
                    last_payment_date = mntripModelArrayList.get(Position).getClosing_date();
                }
                long startTime = 0;
