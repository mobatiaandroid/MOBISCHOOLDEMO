...
class CardPaymentActivity : Activity(), CardPaymentContract.Interactions {
    ...
    override fun onPaymentCaptured(response: JSONObject?) {
        if (response != null) {
            finishWithData(CardPaymentData(CardPaymentData.STATUS_PAYMENT_CAPTURED),response)
        }
    }
    ...
    private fun finishWithData(cardPaymentData: CardPaymentData,response : JSONObject) {
        val intent = Intent().apply {
            putExtra(CardPaymentData.INTENT_DATA_KEY, cardPaymentData)
//            putExtra("jsonResponse", response.toString())
        }
        setResult(Activity.RESULT_OK, intent)
        finish()
    }
